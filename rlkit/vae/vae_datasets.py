...
def transform_data(data):
    num_episodes = data['n_episodes'].item()
    # action = data['action'].reshape((num_episodes, 100, -1))
    key_pts = data['key_pts'].reshape((num_episodes, 100, -1))
    picker_pos = data['picker_pos'].reshape((num_episodes, 100, -1)) # 3 dim picker pos
    state = np.concatenate((picker_pos, key_pts), axis=-1)
...
class VaeDataset(Dataset):
    ...
    def __init__(self,
        ...
        # load the data
        self.data = read_h5_dict(data_path)
